...
@Injectable()
export class Srm2BankBranchService {
    constructor(
        @InjectQueue(SRM2_BANKBRANCH_TRANSPORT)
    ...
    async startJob() {
        ...
        const { id } = await this.queue.add(`推送银行支行 ${startupTime}`, {});
    ...
    async listJob() {
        const jobList = await this.queue.getJobs(null, 0, 100, false);
