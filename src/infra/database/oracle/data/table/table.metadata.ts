 ...
 */
export abstract class TableMetadata<T extends TableEntity> {
    /**
     * PoolAilas
     *
     */
    ...
    createQuery(
        ...
        const { column: pkColumn } = this.primaryKey ?? {};

        if (
            pkColumn &&
            !columnFieldsBindArray.find((col) => col === pkColumn)
        ) {
            columnFieldsBindArray.concat(pkColumn);
        } else {
            columnFieldsBindArray.concat('ROWID');
        }

        const baseSql = `SELECT ${columnFieldsBindArray} \nFROM ${this.schema}.${this.tableName}`;
    ...
    extractCriteriaNative(
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (key === 'id') {
                if (pk && pkColumn) column = pkColumn;
                else column = 'rowid';
            } else column = this.columnMapper[key]?.name ?? key;
    ...
    createCriteriaOffset(
        ...
        // oracle parameters
            ...
            const sql = `SELECT ${columnFieldsBindArray}, ${totalCountField} FROM ${this.tableName} WHERE\n ${whereOptions}`;
            // set statement
